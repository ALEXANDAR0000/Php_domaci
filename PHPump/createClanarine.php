<?php
include_once("config.php");

$clan_id = "";
$datum_pocetka = "";
$datum_kraja = "";
$status = "";


$errorMessage = "";
$successMessage = "";

if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    $clan_id = $_POST["clan_id"];
    $datum_pocetka = $_POST["datum_pocetka"];
    $datum_kraja = $_POST["datum_kraja"];
    $status = $_POST["status"];

    do {
        if (empty($clan_id) || empty($datum_pocetka) || empty($datum_kraja) || empty($status)) {
            $errorMessage = "Sva polja su obavezna";
            break;
        }

        $sql = "INSERT INTO clanarine (clan_id, datum_pocetka, datum_kraja, status) VALUES ('$clan_id', '$datum_pocetka', '$datum_kraja', '$status')";
        $result = $conn->query($sql);

        if (!$result) {
            $errorMessage = "Invalid query: " . $conn->error;
            break;
        }

        $clan_id = "";
        $datum_pocetka = "";
        $datum_kraja = "";
        $status = "";
       
        $successMessage = "Članarina je uspešno dodata";

        header("Location: clanarineCRUD.php");
        exit;
    } while (false);
}
?>

<!DOCTYPE html>
<html>
<head>
    <title>Dodaj</title>
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css">
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/js/bootstrap.bundle.min.js"></script>
</head>
<body>
    <div class="container my-5">
        <h2>Nova Članarina</h2>
        <?php 
        if (!empty($errorMessage)) {
            echo "
            <div class='alert alert-warning alert-dismissible fade show' role='alert'>
                <strong>$errorMessage</strong>
                <button type='button' class='btn-close' data-bs-dismiss='alert' aria-label='Close'></button>
            </div>
            ";
        }
        ?>
        <form method="post">
            <div class="row mb-3">
                <label class="col-sm-3 col-form-label">Član id</label>
                <div class="col-sm-6">
                    <input type="number" class="form-control" name="clan_id" value="<?php echo $clan_id; ?>">
                </div>
            </div>
            <div class="row mb-3">
                <label class="col-sm-3 col-form-label">Datum početka</label>
                <div class="col-sm-6">
                    <input type="date" class="form-control" name="datum_pocetka" value="<?php echo $datum_pocetka; ?>">
                </div>
            </div>
            <div class="row mb-3">
                <label class="col-sm-3 col-form-label">Datum kraja</label>
                <div class="col-sm-6">
                    <input type="date" class="form-control" name="datum_kraja" value="<?php echo $datum_kraja; ?>">
                </div>
            </div>
            <div class="row mb-3">
                <label class="col-sm-3 col-form-label">Status</label>
                <div class="col-sm-6">
                <select class="form-control" name="status" value="<?php echo $status; ?>">
                            <option value="aktivan">Aktivna</option>
                            <option value="istekao">Istekla</option>
                        </select><br>
                </div>
            </div>
            <?php 
            if (!empty($successMessage)) {
                echo "
                <div class='row mb-3'>
                    <div class='offset-sm-3 col-sm-6'>
                        <div class='alert alert-success alert-dismissible fade show' role='alert'>
                            <strong>$successMessage</strong>
                            <button type='button' class='btn-close' data-bs-dismiss='alert' aria-label='Close'></button>
                        </div>
                    </div>
                </div>
                ";
            }
            ?>
            <div class="row mb-3">
                <div class="offset-sm-3 col-sm-3 d-grid">
                    <button type="submit" class="btn btn-primary">Sačuvaj</button>
                </div>
                <div class="col-sm-3 d-grid">
                    <a class="btn btn-outline-primary" href="/PHPump/clanoviCRUD.php" role="button">Otkaži</a>
                </div>
            </div>
        </form>
    </div>
</body>
</html>
